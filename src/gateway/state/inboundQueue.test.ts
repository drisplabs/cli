...
describe('InboundQueue', () => {
	it('enqueues and drains in FIFO order', () => {
		...
		expect(q.enqueue(mkInbound('k1')).kind).toBe('queued');
		expect(q.enqueue(mkInbound('k2')).kind).toBe('queued');
		expect(q.enqueue(mkInbound('k3')).kind).toBe('queued');
	...
	it('treats duplicate idempotency keys as no-ops', () => {
		...
		const q = new InboundQueue(db);
		expect(q.enqueue(mkInbound('k1')).kind).toBe('queued');
		expect(q.enqueue(mkInbound('k1')).kind).toBe('duplicate');
	...
	it('rejects when queue is full', () => {
		...
		expect(q.enqueue(mkInbound('k1')).kind).toBe('queued');
		expect(q.enqueue(mkInbound('k2')).kind).toBe('queued');
		const result = q.enqueue(mkInbound('k3'));
